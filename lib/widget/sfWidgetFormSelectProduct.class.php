  ...
  protected function formatChoices($name, $value, $choices, $attributes) {
    ...
    foreach ($choices as $key => $option) {
      ...
      if ($option instanceof Product) {
        ...
        $inputs[$id] = sprintf('<tr><td><label style="cursor:pointer">%s %s</label></td><td style="text-align: right;">%s</td><td style="text-align: right;">%s</td><td style="text-align: right;">%s</td><td style="text-align: right;">%s</td></tr>', $this->renderTag('input', array_merge($baseAttributes, $attributes)), self::escapeOnce($option->getName()), format_number($option->getEmails()), format_number($option->getDays()), format_currency($option->getPrice(), StoreTable::value(StoreTable::BILLING_CURRENCY)), format_currency($option->getPriceBrutto(), StoreTable::value(StoreTable::BILLING_CURRENCY)));
      }
    }
